...
Гибкий маппинг колонок (RU/EN), авто-классификация и расчёт риска, геопривязка
по центру района если нет координат, и **дедупликация при загрузке** (сверка с
базой по координатам + имени). Ответ — отчёт: `{created, duplicates, errors, …}`.
Импортированные объекты получают `source: imported`, `verification_status: needs_check`.
