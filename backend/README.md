...
### Импорт из источников (ТЗ задача 2)
| Метод | Путь | Описание |
|------|------|----------|
| POST | `/api/import/structures` | Загрузка CSV/Excel (multipart `file`) |
| GET | `/api/import/template.csv` | Скачать шаблон с нужными колонками |

Принимает `.csv / .xlsx / .xls` (включая «грязные» файлы как `датасет.xls`):
пропускает строки-заголовки, **сам находит строку шапки**, сопоставляет русские
колонки по подстроке («Год ввода в эксплуатацию», «Техническое состояние» и т.д.),
переводит `удов./не удов.` в 4 категории, **определяет тип из названия**, и
**геопривязывает по центру района**, если координат нет. **Дедупликация при
загрузке** — по координатам (350 м) и по имени+типу. Ответ — отчёт
`{created, duplicates, errors, skipped_empty, …}`. Импортированные объекты
получают `source: imported`, `verification_status: needs_check` и сразу видны
...
- **Risk Score — единая риск-модель** (`services/risk_score.py`, экспертная из ТЗ):
  состояние 0/25/50/80 + возраст 0/10/20/30 + давность осмотра 0/15/30 + аварии
  0/10/20 (cap 100), уровень + причины + цвет. **Один источник риска во всём
  приложении** — каталог, карта, фильтры, аналитика, карточка, `/top-risk`,
  `/risk`, `/risk-score`. В БД хранится `risk_score` (float) + `risk_level` (код).
- **Priority Score** (`services/priority.py`) — отдельная метрика «срочность
  осмотра» (риск + значимость + просрочка) + рекомендуемый срок.
- **Сезонность осмотра** (ТЗ задача 5) — паводок-чувствительные объекты (плотины,
  дамбы, шлюзы, водозаборы) осматриваются чаще и до весеннего половодья
  (`services/risk_engine.py` оставлен только как планировщик даты осмотра).
